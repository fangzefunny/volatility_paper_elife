...
def trial_step(
            # sequence
            info_A_tm1,info_A_t, # externally provided to function on each trial
    ...
    logit1 = T.log( 1.0 - choice_kernel_tm1) - \
             Binv_t * mag_1_t - T.log( choice_kernel_tm1) 
    logit1 = T.switch( logit1 < -10, -10, logit1)
    pi1_s1_t = 1.0 / ( 1.0 + T.exp(logit1))

    # π2 = π(c=1|s=0) = qt / [qt + (1-qt)*exp(βM0)] 
    logit0 = Binv_t * mag_0_t + T.log( 1.0 - choice_kernel_tm1) - \
            T.log( choice_kernel_tm1) 
    logit0 = T.switch( logit0 < -10, -10, logit0)
    pi1_s0_t = 1.0 / ( 1.0 + T.exp(logit0))
...
def create_choice_model(X,Y,param_names,Theta,gen_indicator=0,B_max=10.0,nonlinear_indicator=0):
    ...
    ############ filll #############
    staring_pi1_s1 = T.ones(NN)*.5
    staring_pi1_s0 = T.ones(NN)*.5         
    ################################ 
    #import pdb; pdb.set_trace()

    (choice,outcome_valence,prob_choice,choice_val,estimate_r,choice_kernel,
    lr,lr_c,Gamma,Binv,Bc,mdiff,eps,
    ############ filll #############
    pi1_s1, pi1_s0,        
    ################################ 
                                            ...
                                            outputs_info=[starting_choice,starting_outcome_valence, # observables
                                                ...
                                                starting_lr,starting_lr_c,starting_Gamma,starting_Binv,starting_Bc,starting_mdiff,starting_eps,
                                                ############ filll #############
                                                staring_pi1_s1, staring_pi1_s0 
                                                ################################ 
    ...
    return((choice,outcome_valence,prob_choice,choice_val,estimate_r,choice_kernel,lr,lr_c,Gamma,Binv,Bc,mdiff,eps,
            ############ filll #############
            pi1_s1, pi1_s0,
            ################################ 
            ), updates)
...
def combined_prior_model_to_choice_model(X,Y,param_names,model,
    ...
    with model:
        ...
        pi1_s1, pi1_s0,    
        ################################ 
        ), updates = create_choice_model(X,Y,param_names,model.Theta,gen_indicator=0,B_max=B_max)
        ...
        if save_state_variables:
            ...
            ############ filll #############
            pi1_s1 = pm.Deterministic('pi1_s1', pi1_s1)
            pi1_s0 = pm.Deterministic('pi1_s0', pi1_s0)
            ################################ 
...
def create_gen_choice_model(X,Y,param_names,B_max=10.0,seed=1,nonlinear_indicator=0):
    ...
    pi1_s1, pi1_s0,        
    ################################ 
    ), updates = create_choice_model(X,Y,param_names,Theta,gen_indicator=1,B_max=B_max)
